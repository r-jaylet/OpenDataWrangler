"""OpenDataWrapper
    ...
    Summary
    -------
        Manipulation des données sur les locaux de la base MAJIC
        ...
        try:
            ...
            if locaux_df is not None:
                ...
                code_droit_proprietaires = pd.DataFrame(
                    data={self.COLUMN_CODE_DROIT: dict_code_droit_proprietaires.keys(),
                          'nomenclatureCodeDroit': dict_code_droit_proprietaires.values()})
                ...
                locaux_df[self.COLUMN_NAME_SECTION] = locaux_df[self.COLUMN_NAME_SECTION].apply(
                ...
                locaux_df = locaux_df[[self.COLUMN_NAME_ID_PARCELLE] +
                                      [col for col in locaux_df.columns if col != self.COLUMN_NAME_ID_PARCELLE]]

                # correction du type des adresses des locaux
                locaux_df[self.COLUMN_NAME_NUMERO_VOIRIE] = pd.to_numeric(
                    locaux_df[self.COLUMN_NAME_NUMERO_VOIRIE], errors='coerce').fillna(0).astype(int)
                locaux_df[self.COLUMN_NAME_CODE_COMMUNE] = locaux_df[self.COLUMN_NAME_DEPARTEMENT].astype(
                    str) + locaux_df[self.COLUMN_NAME_CODE_COMMUNE].astype(str).apply(lambda x: x.zfill(3))
                locaux_df[self.COLUMN_NAME_ENTREE] = pd.to_numeric(
                    locaux_df[self.COLUMN_NAME_ENTREE], errors='coerce').fillna(0).astype(int)
                locaux_df[self.COLUMN_NAME_NIVEAU] = pd.to_numeric(
                    locaux_df[self.COLUMN_NAME_NIVEAU], errors='coerce').fillna(0).astype(int)
                locaux_df[self.COLUMN_NAME_PORTE] = pd.to_numeric(
                    locaux_df[self.COLUMN_NAME_PORTE], errors='coerce').fillna(0).astype(int)
